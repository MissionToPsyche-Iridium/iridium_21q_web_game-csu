...
@onready var drillspeed: int = 2 #time (sec) it takes for a drill to fully break a block.
@onready var drillstr: int = 0 #TODO, not used anywhere right now.

@onready var dronespeed: int = 200 #How fast the drone moves.

@onready var tubespeed: int = 2 #Time (sec) between objects leaving the drone and entering the mothership.
